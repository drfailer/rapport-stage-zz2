...
l'intégration du projet de test dans le projet commun. À noter qu'il y a un
autre répertoire \verb|CFake| qui lui se trouve à la racine du projet, au même
niveau que \verb|commun|. Ce dernier contient la simulation des interfaces pour
le stockage. Étant donné que le stockage est un élément assez général, il a été
mis à part. Comme on peut le voir ici, tous les fichiers ayant un lien entre eux
...
comme le nom du fichier à lire, le nombre de répétitions, \dots
...
intéressant car il permet de mieux repérer l'origine des problème car le code
testé est isolé, ce qui fait que les échecs des tests sont dus uniquement aux
...
de faire à la fois des tests unitaires et des tests d'intégration. Pour mettre
...
sont supprimées et sauvegarde de la trame reçue ainsi que l'envoi de la réponse
...
\begin{itemize}
  \item[$\bullet$] \verb|Control|: Cette fonctions est utilisée avec la
    simulation bas niveau qui est la première solution crée. Cette fonction est
    une machine qui se compose de trois états. Dans le premier état, la machine
    fait la vérification de l'adresse sur la commande envoyée sur le bus par le
    code testé. Si cette adresse ne correspond pas à l'adresse du composant
    simulé, la machine s'arrête. Dans le cas contraire, la machine commence par
    ...
    maître en fonctions des données qu'il a reçu, en utilisant les fonction du
    compilateur. % TODO: code en annexe
  \item[$\bullet$] \verb|Run|:
  \item[$\bullet$] \verb|HandleResponse|:
\end{itemize}
