...
{
    name=TAP,
    description={(Test Anything Protocol) protocol qui permet d'assuer la
...
Qt, merci à Denis GREFFET, Manel ELICHOOUIA et Quentin LAGIER pour leurs aide
concernant la mise en place de l'outil de test sur dans le code C++. Enfin,
...
Le défit de cette tâche réside dans le fait que les composants électroniques
connectés aux cartes devaient être simulés pour permettre de tester les
différents programmes. De plus, les composants électroniques sont interfacé avec
des protocoles qui n'étaient pas connus avant le stage et qu'il a donc fallut
découvrir.\\
...
accès aux cartes. Pour rendre cela possible, les composants électronique
...
événementiel) ainsi que sur la partie C++ (partie Qt).\\
...
l'organisation du projet ainsi que son fonctionnement général pour permettre une
meilleur compréhension des choix qui ont était fait durant le développement.
...
aurons détailler le fonctionnement de l'outil et de la simulation, nous verrons
...
l'entreprise est que ce sont les développeurs qui testent leurs code. Des tests
...
l'entreprise ne sont pas nécessairement formés à l'utilisation de ce types
...
frameworks terminée, il a fallut trouver des critères pour les comparer.\\
...
complexe va nécessité la conception de fonctions et macros (pour réduire la
...
l'export des résultats des tests dans différents format comme \gls{tap} ou XML
(utile pour faire des rapports) ou encore des générateur de nombres pseudo
aléatoires pour faire des tests avec des entrée aléatoires,\dots Une autre
...
beaucoup de macros à aussi été pris en compte car bien que ces dernières
...
sont très compliquées à déboguer). Le frameworks qui a été choisi possède ce
...
de simple tests) mais possède beaucoup de fonctions d'assertion. En terme
...
Le framework choisit possède donc beaucoup de qualités mais aussi quelques
...
y ai encore des défaut qui seront certainement corrigés au fil des années de
développement.
%}}}
%}}}
...
l'écran Cctalk. Ici, on test la fonction \verb|DISPLAYCCTALK_Print| qui permet
...
être envoyée. Ensuite on lance le composant simulé qui va récupérer la trame est
envoyer une réponse. Enfin, on peut tester que les données reçues par le
composants sont correctes.
...
l'on peut facilement avoir accès au données transmises aux composants.
\\~\\
...
complexe à mettre en place car on a pas accès à tout. Dans ce cas, il faut
...
des historiques ainsi qu'à leurs sauvegarde sur le serveur de la société.
%}}}
...
partie Qt, QtCreator n'a pas été utilisé non plus car les projet ne sont pas
fait pour être compilé avec gcc. Pour utiliser cet outil, il aurait fallut tout
reconfigurer ce qui n'était pas souhaitable étant donné le fait que cela aurait
complexifier la création de l'outil de test. L'environnement de développement
...
fourni aussi une complétion de texte intelligente. Cet outil permet à n'importe
...
rapidement tout en minimisant les mouvement de l'utilisateur. En effet, il
...
événementiel. En effet, la branche principale avait plusieurs mois d'avances sur
la branche secondaire et beaucoup de changements avaient été fait. La fusion des
deux branche a nécessité près de deux semaines de travail et les tests ont
...
accès à certains élément protégés des classes a été utilisée pour permettre de
...
que celle du C car ici, un nouveau test à été ajouté. En effet, les développeurs
...
TODO: parler du fait qu'on est obliger d'utiliser des wait en Qt
