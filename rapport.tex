...
esclave. L'interface du Cctalk est aussi utilisée avec le TCPIP. En effet, le
...
sont pas directement connectés et pour ce faire on utilise le protocole TCPIP
qui est le plus adapté. Le code de la société propose une abstraction qui permet
de gérer tous les composants en utilisant seulement l'interface du Cctalk. Pour
rendre cela possible, le code utilise des drivers qui fonctionnent à l'aide de
...
Le protocole MDB a été créé dans les années 1980 par la société CoinCo et a au
départ été très utilisé dans les distributeurs automatique de Coca-Cola
\cite{enwiki:1094073752}. Le protocole est devenu \textit{open-source} en 1992
et la National Automatic Merchandising Association (NAMA) a réalisé la première
version du standard en 1995.
...
La transmission des informations MDB ne se fait pas sur un octet mais sur neuf
bits. Parmi ces neuf bits, il y a huit bits qui permettent de transmettre des
données et le dernier bit et un bit de mode. Lorsque le bit de mode est à 0, les
huit premiers bits contiennent des données. S'il est à 1, alors les huit
premiers bits peuvent contenir l'adresse d'un composant cible (l'adresse du
composant auquel s'adresse le maître), une commande spéciale ou un checksum. Les
commandes spéciales sont ACK (acquittement), NAK (non acquittement) et RET
(demande de répétition). Un octet d'adresse contient généralement aussi une
commande: un \textbf{ou} logique est fait entre l'adresse du composant et la
commande à envoyer. À noter que le standard spécifie des adresses spécifiques
pour chaque type de périphérique. Par exemple, un accepteur de billet doit avoir
pour adresse \verb|0x30|. Les commandes envoyées avec l'adresse permettent de
donner des ordres aux périphériques et chaque type de périphérique possède son
propre jeux de commandes. Les commandes correspondes à des catégories assez
génériques, par exemple pour l'accepteur de billet, \verb|0x03| permet de faire
un poll. Ensuite, pour spécifier une action plus spécifique à réaliser on envoi
à nouveau neuf bits avec le bit de mode à 0 et une sous-commande dans les
données.

Un exemple de communication.

TODO: \cite{mdbdoc} et \cite{enwiki:1094073752}
...
d'étapes et chaque étape contient TODO: REVOIR LA STRUCT DU SCÉNARIO
%}}}
%}}}
