...
commandes. Les commandes correspondes à des catégories assez génériques, par
...
spécifier une action plus spécifique à réaliser on envoi à nouveau neuf bits
avec le bit de mode à 0 et une sous-commande dans les données.
...
le composant en utilisant la commande \textit{poll} (commande \verb|0x02|). Les
...
est à 0 et la huit bits suivant contient la commande \verb|0x03|, ce qui permet
de démarrer une session (échange de trames). Une fois la session démarrée, la
carte va demander s'il y a une ventre à traiter, pour ce faire, on utilise la
commande \textit{Vend} (\verb|0x03|) et cette dernière s'accompagne d'une
sous-commande \textit{Vend Request} (\verb|0x00|). On peut noter ici que pour un
même code, la commande correspondante sera différente en fonction de l'émetteur
de la trame (maître ou esclave). La nouvelle trame sera donc composée de 18
bits: \verb|0x113| (bit de mode à 1, adresse du composant \verb|0x10| et
commande \verb|0x03|) \verb|0x000| (bit de mode à 0 et sous-commande
\verb|0x00|). Ensuite le périphérique doit répondre par la commande ACK, donc
ici, le bit de mode est à 1 et les huit bits suivants sont nuls. Comme mentionné
plus haut, le reste du scénario est décrit dans la documentation du MDB.

\subsubsection{Le code de l'entreprise}

\subsubsection{Émulation des composants}

La simulation des composants interfacés en MDB est dans la continuité de ce qui
a été fait pour le Cctalk. Là aussi, on a un fichier par composant, et les
fichiers ont une forme similaire.
