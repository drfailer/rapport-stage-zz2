...
insérée et la queue à la dernière. Les CDB sont sauvegardé en mémoire sur des
eeproms ou des flashs en fonction de la configurions. Il y a une seule interface
(fichier d'entête) pour deux implémentation différentes utilisant les deux types
de périphérique de stockage et on peut passer d'une implémentation à une autre
avec la configuration. Il est important de pouvoir tester les deux
implémentation, par contre, étant données que les fonctions ont le même nom, on
ne peut pas compiler tous les fichier (une interface et deux implémentations).
C'est pour cette raison que l'on génère deux exécutables différent, où chaque
exécutables est compilé avec une configuration différente. Comme expliqué dans
la section \ref{cmake}, pour passer d'une configuration à une autre, on utilise
des constantes préprocesseur définie à la compilation.

TODO: fonctionnement des CDB.\\
TODO: réutilisation du stockage.\\
TODO: exemple de test

\subsection{Sauvegarde des historiques}

Maintenant que nous avons traité l'utilité et le fonctionnement des CDB,
étudions le problème de l'envoi des historiques. À noter que c'est un problème
concernant l'envoi des historiques de paiement sur le \textit{CKWash} (serveur
de CKsquare) qui a donné aux développeurs l'idée de ce stage.\\
...
historiques, on utilise une protocole créé par la société qui est le protocole
...
traités durant le stage car cela aurait prise trop de temps.
...
Mdb, la fonction \verb|Control| du fichier n'est pas utilisée et les la fonction
...
envoyer tous les éléments enregistrés dans le CDB. On peut ensuite tester si les
commandes envoyées correspondes aux données qui avaient été enregistrées en
vérifiant que les identifiants stockés dans les trames sont les mêmes que ceux
des éléments du CDB. À noter qu'ici, un test assez réaliste a été écrit pour
vérifier que l'outil permet bien de faire un test capable de détecter le
problème qui a pousser la société à créer ce sujet de stage. Le problème été
survenu plusieurs fois à cause de mauvaises manipulation de git. Il venait d'une
erreur de configuration qui faisait que dans la fonction \verb|Control| de l'envoi
des historiques, une fonction était appelée à la place d'une autre. Ce problème
était assez difficile à détecter avec de simple puisqu'il apparaissait
uniquement lorsque le CDB était remplit plusieurs fois (problème d'index dans le
CDB). Le test qui a été écrit remplit le CDB, envoi tous les historiques et
refait la même opération en vérifiant à chaque fois que tous les éléments sont
envoyés et cela permet bien de détecter le problème.
...
Dans cette section nous allons voir la solution comment a été mise en place la
pipeline Gitlab.

La configuration de la pipeline contient deux \textit{jobs}, un \textit{job} de
compilation et un \textit{job} de test. Il y a une contrainte de précédente
...
beaucoup de paquets à installer. Le projet \verb|dev_pic| est dépendant du
projet \verb|commun_global| qui comme expliqué en début de rapport correspond au
code partagé entre tous les projets, il faut donc un moyen d'accéder à ce
projet. Là, il y a deux solution, la première consiste à utiliser un
\gls{smodg}. Le défaut de cette méthode est qu'elle nécessite la gestion d'un
sous-module dans les communs. De plus, cette solution n'est pas très logique et
complexifierait énormément la gestion des sous modules car le projet
