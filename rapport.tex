...
Comme mensionné dans le paragraphe précédent, un CDB est une une base de donnée
...
nouveau historiques sur le \textit{CKWash} et les supprimer des CDB. Pour
transmettre les historiques, on utilise une protocole créé par la société qui
est le protocole \textbf{CKspro}. Ici, seul une version haut niveau de la
simulation à été mise en place car le code permettant de gérer la transmission
des données utilisait directement des éléments bas niveau comme le TCPIP. C'est
éléments étant relativement complexes et assez peu, ils n'ont pas été traités
durant le stage car cela aurait prise trop de temps.

Pour pouvoir tester l'envoi des commandes, une version modifié du fichier chargé
de la gestion du protocole CKspro a été créée. Comme pour le Cctalk et le Mdb,
la fonction \verb|Control| du fichier n'est pas utilisée et les la fonction
...
tester si les commandes sont bien envoyées, on remplit un CDB et on appelle la
fonction \verb|Control| du fichier \verb|CksproHisSend.c| qui va envoyer tous
les éléments enregistrés dans le CDB. On peut ensuite tester si les commandes
envoyées correspondes aux données qui avaient été enregistrées.
...
\subsection{Les avantages du C++ pour la simulation}
