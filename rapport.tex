...
Étant donné le fait que le projet a été développé sur Linux, l'IDE des
développeurs de la société (MPLAB) n'a pas été utilisé. En effet, il était
préférable de développer sous Linux étant donné le fait que la pipeline ne
s'exécute pas sur Windows. Le problème est que l'IDE et les compilateurs de
Microship ne fonctionnent pas bien sur Linux. Pour la partie Qt, QtCreator n'a
pas été utilisé non plus car les projet ne sont pas fait pour être compilé avec
gcc. Pour utiliser cet outil, il aurait fallut tout reconfigurer ce qui n'était
pas souhaitable étant donné le fait que cela aurait complexifier la création de
l'outil de test. L'environnement de développement utilisé durant le stage était
donc \textit{neovim}.
